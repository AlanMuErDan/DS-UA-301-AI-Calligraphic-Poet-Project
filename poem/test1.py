...
ds = []
with open("./data/test1_updated.txt", "r") as f:
    for line in f:
        line = line.strip()
        if line:
            ds.append(line)
...
for i in range(5): 
    ...
    fo = open("./results/poem-top-"+str(k)+"/poem-out"+str(i+1)+".txt", "w")
    while idx < len(ds):
...
for i in range(6): 
    ...
    # 打开同一个文件，以追加模式写入
    with open("./results/poem-top-"+str(k)+"/poem-out.txt", "a", encoding="utf-8") as fo:
        while idx < len(ds):
            lb = ds[idx:idx + batch_size]
            cplb = []
            for line in lb:
            ...
            # 将每个batch的输入和生成的输出结果写入同一个文件
            for i, line in enumerate(cplb):
                r = ''.join(res[i])
                print(line)
                print(r)

                # 将输入和生成结果写入文件，按照每个batch中的输出
                fo.write(line + "\t" + r + "\n")
